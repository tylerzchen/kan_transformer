...
class Transformer(Module):
    ...
    def __init__(
        ...
        factory_kwargs = {key: value for key, value in {"device": device, "dtype": dtype}.items() if value is not None}
        super().__init__()
...
class TransformerEncoder(Module):
    ...
    Each encoder layer uses self-attention and a feedforward network implemented using Kolmagorov-Arnold
    Neural Network (KAN).
    ...
    def forward(
        ...
        if not why_not_sparsity_fast_path:
            tensor_args = (
                ...
                first_layer.kan_ffn.fc1.weight,
                first_layer.kan_ffn.fc1.bias,
                first_layer.kan_ffn.fc2.weight,
                first_layer.kan_ffn.fc2.bias,
            )
            _supported_device_type = [
                "cpu",
                "cuda",
                backend_registration._privateuse1_backend_name,
            ]
...
class TransformerEncoderLayer(Module):
    ...
    This implementation uses a Kolmogorov-ARnold Neural Network (KAN) for the feedforward layer.
    Args:
    ...
    def forward(
        ...
        if not why_not_sparsity_fast_path:
            tensor_args = (
                ...
                self.norm2.weight,
                self.norm2.bias,
                self.kan_ffn.fc1.weight,
                self.kan_ffn.fc1.bias,
                self.kan_ffn.fc2.weight,
                self.kan_ffn.fc2.bias,
            )
